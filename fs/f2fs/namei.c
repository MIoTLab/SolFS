...
struct inode *f2fs_new_version_inode(struct inode *host_inode)
{
	...
	inode = new_inode(host_inode->i_sb);
	if (!inode)
		return ERR_PTR(-ENOMEM);
	...
	/////////////
	// replace 	inode_init_owner(&init_user_ns, inode, dir, mode);
	inode->i_uid = host_inode->i_uid;
	inode->i_gid = host_inode->i_gid;
	inode->i_mode = host_inode->i_mode;
	////////////
	...
	F2FS_I(inode)->i_projid = F2FS_I(host_inode)->i_projid;
	...
	F2FS_I(inode)->i_flags = F2FS_I(host_inode)->i_flags;
